...
test "Arena.alloc" {
    ...
    const i = try arena.alloc(allocator);
...
test "Arena.append" {
    ...
    const rb = Example.default();
    var i: usize = 0;
    while (i < 10000) : (i += 1) {
        const index = try arena.append(rb);
...
test "Arena.remove" {
    ...
    while (i < 4) : (i += 1) {
        ...
        const before_delete = arena.get(index).?;
        const deleted_entry = arena.remove(index).?;
...
test "Arena.clear" {
    ...
    const index1 = try arena.append(Example.default());
    const index2 = try arena.append(Example.default());
    const index3 = try arena.append(Example.default());
    arena.clear();
...
test "Arena.mutate" {
    ...
    const index = try arena.append(Example.default());
