...
pub const SafePointer = struct {
    ...
    pub const null_pointer = SafePointer{ .address = 0, .type_id = "" };
    ...
    pub fn cast(self: SafePointer, comptime T: type) T {
        ...
        if (!std.mem.eql(u8, self.type_id, @typeName(T))) {
            if (builtin.mode == .Debug) {
                std.debug.panic("Type mismatch: Expected {s}, but got {s}!", .{ @typeName(T), self.type_id });
            } else {
    ...
    pub fn tryCast(self: SafePointer, comptime T: type) ?T {
        ...
        if (self.isNull())
            return null;
        return if (std.mem.eql(u8, self.type_id, @typeName(T)))
